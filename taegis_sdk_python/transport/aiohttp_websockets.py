...
class AIOHTTPWebsocketsTransport(AsyncTransport):
    ...
    def __init__(  # pylint: disable=too-many-locals, too-many-statements
        ...
        *,
        ...
        self.init_payload: Dict[str, Any] = init_payload or {}
    ...
    async def _after_connect(self):
        ...
        response_headers = (
            self.websocket._response.headers  # pylint: disable=protected-access
        )
    ...
    async def _receive(self) -> str:
        ...
        if ws_message.type is WSMsgType.BINARY:
    ...
    async def _close_coro(
        self, e: Exception, clean_close: bool = True
        ...
        try:
            ...
            try:
                ...
                log.warning(
                    f"_close_coro cancel keep alive task exception: {repr(exc)}"
                )
            ...
            try:
                ...
                log.warning(f"_close_coro close_hook exception: {repr(exc)}")
            ...
            if clean_close:
                ...
                try:
                    ...
                    log.warning(f"Ignoring exception in _clean_close: {repr(exc)}")
            ...
            try:
                ...
                log.warning(f"_close_coro websocket close exception: {repr(exc)}")
            ...
            if (
                ...
                try:
                    ...
                    log.warning(f"_close_coro session close exception: {repr(exc)}")
            ...
            try:
                ...
                log.warning(
                    f"_close_coro cancel receive data task exception: {repr(exc)}"
                )
    ...
    async def wait_closed(self) -> None:
        """Wait to close the transport properly."""
